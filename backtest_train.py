#!/usr/bin/env python3
"""
backtest_train.py — Same as backtest.py but runs on TRAIN split only.

Use this for optimization so the agent never sees validation data.
Walk-forward validation is the real test.
"""
...
COMMISSION = 0.001
RESULTS_FILE = "results_train.json"
TIME_BUDGET_SEC = 300
...
class Portfolio:
    ...
    def buy(self, ticker: str, price: float, date, position_size: float):
        ...
        if cost > self.cash:
            return
        self.cash -= cost
        self.positions[ticker] = {"shares": shares, "entry_price": price, "entry_date": str(date)}
    ...
    def sell(self, ticker: str, price: float, date, reason: str = "signal"):
        ...
        self.trades.append({
            "ticker": ticker, "entry_date": pos["entry_date"], "exit_date": str(date),
...
def run_backtest():
    ...
    # KEY DIFFERENCE: load TRAIN split
    print("Loading TRAIN data (optimizer must not see validation)...")
    universe = load_universe("train")
    if not universe:
    ...
    print("Running simulation on TRAIN data...")
    ...
    results = {
        ...
        "summary": f"[TRAIN] Sharpe={sharpe:.3f} | Return={total_return*100:.1f}% | MaxDD={max_drawdown*100:.1f}% | Trades={len(trades)} | WinRate={win_rate*100:.0f}%",
    }

    print("\n" + "=" * 60)
    print("BACKTEST RESULTS (TRAIN SPLIT)")
    print("=" * 60)
