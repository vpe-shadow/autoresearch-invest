...
Exp 3: Momentum + ATR-based dynamic stops
"""
...
MOMENTUM_PERIOD = 20
MOMENTUM_THRESHOLD = 0.05  # 5% minimum return
ATR_PERIOD = 14
...
STOP_LOSS = -0.07     # -7% stop loss (fallback)
...
def compute_signals(df: pd.DataFrame) -> pd.Series:
    """
    Momentum strategy with ATR-aware risk management
    """
    close = df["Close"].copy()
    high = df["High"].copy()
    low = df["Low"].copy()
    ...
    # Buy: strong momentum + reasonable volatility (ATR < 8%)
    buy_condition = (momentum > MOMENTUM_THRESHOLD) & (atr_pct < 0.08)
